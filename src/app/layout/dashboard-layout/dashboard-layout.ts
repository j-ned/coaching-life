  ...
  template: `
    ...
    <aside class="w-full md:w-64 bg-slate-900 md:min-h-screen shrink-0 flex flex-col">
      <div class="h-20 flex items-center px-6 border-b border-slate-800">
        <a routerLink="/" class="text-white font-semibold text-xl tracking-tight"
          >Coaching Life Dashboard</a
        >
      </div>
      ...
      <div class="p-4 border-t border-slate-800 space-y-1">
        ...
        <button
          (click)="logout()"
          class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors"
        >
    ...
    <main class="grow flex flex-col min-h-screen">
      <header
        class="bg-white h-20 border-b border-slate-200 flex items-center px-8 shadow-sm justify-between"
      >
