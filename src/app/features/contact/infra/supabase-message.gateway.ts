...
import type { Message, MessageStatus, MessageSubmission, SendMessageData } from '../domain/models/message.model';
...
export class SupabaseMessageGateway extends MessageGateway {
  ...
  send(data: SendMessageData): Observable<MessageSubmission> {
    return from(
      ...
      catchError(() =>
        of({ success: false, message: "Une erreur est survenue lors de l'envoi. Veuillez réessayer." }),
      ),
    );
  }
