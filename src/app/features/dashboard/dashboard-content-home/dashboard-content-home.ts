...
import { DEFAULT_HERO, DEFAULT_HOME_CTA, DEFAULT_HOME_SERVICES } from '../../content/domain/models/default-content';
import type { HeroSettings, HomeCTASettings, HomeServicesSettings } from '../../content/domain/models/site-settings.model';
  ...
  template: `
    <div class="mb-8 flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Modifiez le contenu visible sur votre page d'accueil</p>
      </div>
    </div>
    ...
    @if (isLoading()) {
      ...
      <!-- Toast feedback -->
      @if (saveSuccess()) {
        <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl flex items-center gap-3 animate-fade-in" role="alert">
      ...
      <div class="space-y-6 max-w-3xl pb-24">
        <!-- Hero section -->
        <form [formGroup]="heroForm" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div class="bg-gradient-to-r from-brand-500 to-brand-400 px-6 py-4 flex items-center gap-3">
          ...
          <div class="p-6 space-y-5">
            <div>
              <label for="heroTitle" class="block text-sm font-medium text-slate-700 mb-1.5">Titre principal <span aria-hidden="true" class="text-red-400">*</span></label>
              <input id="heroTitle" type="text" formControlName="title" placeholder="Ex : Révélez votre plein potentiel intérieur"
            ...
            <div>
              <label for="heroSubtitle" class="block text-sm font-medium text-slate-700 mb-1.5">Sous-titre <span aria-hidden="true" class="text-red-400">*</span></label>
              <textarea id="heroSubtitle" formControlName="subtitle" rows="3" placeholder="Décrivez en quelques mots votre accompagnement..."
            ...
            <div class="grid sm:grid-cols-2 gap-4">
              <div>
                <label for="ctaPrimaryText" class="block text-sm font-medium text-slate-700 mb-1.5">Bouton principal</label>
                <input id="ctaPrimaryText" type="text" formControlName="ctaPrimaryText" placeholder="Ex : Prendre rendez-vous"
                       class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all" />
                <p class="text-xs text-slate-400 mt-1">Scrolle vers la section contact</p>
              </div>
              <div>
                <label for="ctaSecondaryText" class="block text-sm font-medium text-slate-700 mb-1.5">Bouton secondaire</label>
                <input id="ctaSecondaryText" type="text" formControlName="ctaSecondaryText" placeholder="Ex : Découvrir les services"
                       class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all" />
            ...
            <div>
              <label for="heroImageAlt" class="block text-sm font-medium text-slate-700 mb-1.5">Description de l'image</label>
              <input id="heroImageAlt" type="text" formControlName="imageAlt" placeholder="Ex : Cabinet de coaching apaisant"
                     class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all" />
        ...
        <!-- Services section -->
        <form [formGroup]="servicesForm" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div class="p-6 space-y-5">
            <div>
              <label for="servicesBadge" class="block text-sm font-medium text-slate-700 mb-1.5">Badge</label>
              <input id="servicesBadge" type="text" formControlName="badge" placeholder="Ex : MES ACCOMPAGNEMENTS"
            ...
            <div>
              <label for="servicesTitle" class="block text-sm font-medium text-slate-700 mb-1.5">Titre <span aria-hidden="true" class="text-red-400">*</span></label>
              <input id="servicesTitle" type="text" formControlName="title" placeholder="Ex : Un accompagnement adapté à chaque besoin"
                     class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                     aria-required="true" />
            </div>

            <div>
              <label for="servicesSubtitle" class="block text-sm font-medium text-slate-700 mb-1.5">Sous-titre</label>
              <textarea id="servicesSubtitle" formControlName="subtitle" rows="2" placeholder="Décrivez brièvement vos spécialités..."
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all resize-none"></textarea>
            </div>
          </div>
        </form>

        <!-- CTA section -->
        <form [formGroup]="ctaForm" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div class="bg-gradient-to-r from-emerald-500 to-emerald-400 px-6 py-4 flex items-center gap-3">
            <div class="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center text-white">
              <app-icon name="phone" size="md" />
            </div>
            <div>
              <h3 class="text-white font-semibold">Section Appel à l'action</h3>
              <p class="text-white/80 text-xs">Le texte qui invite vos visiteurs à vous contacter</p>
            </div>
          </div>

          <div class="p-6 space-y-5">
            <div>
              <label for="ctaBadge" class="block text-sm font-medium text-slate-700 mb-1.5">Badge</label>
              <input id="ctaBadge" type="text" formControlName="badge" placeholder="Ex : PASSEZ À L'ACTION"
            ...
            <div>
              <label for="ctaTitle" class="block text-sm font-medium text-slate-700 mb-1.5">Titre <span aria-hidden="true" class="text-red-400">*</span></label>
              <input id="ctaTitle" type="text" formControlName="title" placeholder="Ex : Prêt(e) à commencer ?"
                     class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                     aria-required="true" />
            </div>

            <div>
              <label for="ctaSubtitle" class="block text-sm font-medium text-slate-700 mb-1.5">Sous-titre</label>
              <textarea id="ctaSubtitle" formControlName="subtitle" rows="2" placeholder="Encouragez vos visiteurs à passer à l'action..."
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all resize-none"></textarea>
            </div>
          </div>
        </form>
      </div>
